...
test("the public site exposes complete indexable metadata", () => {
  ...
  assert.match(indexSource, /rel="canonical" href="https:\/\/arcastudio2025\.com\/"/);
...
test("robots, sitemap, logo and social image are deployable public assets", () => {
  for (const asset of [
    "public/robots.txt",
    "public/sitemap.xml",
  ...
  assert.match(read("public/robots.txt"), /Sitemap: https:\/\/arcastudio2025\.com\/sitemap\.xml/);
  assert.match(read("public/sitemap.xml"), /<loc>https:\/\/arcastudio2025\.com\/<\/loc>/);
});

test("the www hostname redirects permanently to the canonical domain", () => {
  const netlifySource = read("netlify.toml");

  assert.match(netlifySource, /from = "https:\/\/www\.arcastudio2025\.com\/\*"/);
  assert.match(netlifySource, /to = "https:\/\/arcastudio2025\.com\/:splat"/);
  assert.match(netlifySource, /status = 301/);
});

test("only the first visual panel is assigned the primary heading", () => {
  ...
  assert.match(sectionsSource, /headingLevel=\{panelIndex === 0 \? 1 : 2\}/);
  assert.match(heroTitleSource, /headingLevel === 1 \? "h1" : "h2"/);
});
