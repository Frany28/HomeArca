...
test("tablet and mobile galleries use a horizontal photo carousel", () => {
  ...
  assert.match(mobileCarouselSource, /overflow-clip/);
...
test("tablet and mobile layouts cannot keep the desktop expansion stage visible", () => {
  ...
  assert.match(
    gallerySource,
    /isFeaturedCarouselLayout\(\) \|\|[\s\S]*!stage/,
  );
  assert.match(
    gallerySource,
    /!stage \|\| progress <= 0 \|\| isFeaturedCarouselLayout\(\)/,
  );
  assert.match(
    gallerySource,
...
test("mobile Featured preserves the surrounding Figma spacing model", () => {
  assert.match(
